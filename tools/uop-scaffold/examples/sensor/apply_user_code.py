...
Each file in user_code/ is named ``<target>.<region>.c`` where target is
``uop`` (the generated ``<uop>.c``) or ``harness`` (``<uop>_harness.c``),
...
Usage: apply_user_code.py <gen-dir> <uop-name>
"""
import re
import sys
...
def main():
    ...
    uc_dir = Path(__file__).parent / "user_code"
    applied = []
